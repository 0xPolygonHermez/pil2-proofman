...
//! `proofman_fields::Transcript` with extension-field challenges.

use crate::error::MlError;
use crate::hypercube::Ext;
use proofman_fields::{hash_state, new_transcript, Field, Goldilocks, PrimeField64, TranscriptDyn};
...
impl MlHashFamily {
    /// The `proofman_fields` hash-family id string (for `new_transcript` / `hash_state`).
...
impl MlTranscript {
    ...
    /// the AIR's hash so prover and verifier agree.
...
mod tests {
    ...
    fn grind_roundtrip_and_rejection() {
        ...
        let mut tw = MlTranscript::new(MlHashFamily::Poseidon2);
        seed(&mut tw);
        assert!(!tw.verify_grind(nonce.wrapping_add(1), bits), "a wrong nonce must be rejected");
    }

    /// After grind/verify_grind the two transcripts stay in lock step.
    #[test]
    ...
    /// Same absorptions ⇒ same challenges; a different absorption ⇒ different challenge.
    #[test]
    fn challenges_are_deterministic_and_binding() {
        let mut a = MlTranscript::new(MlHashFamily::Poseidon2);
        let mut b = MlTranscript::new(MlHashFamily::Poseidon2);
        let mut c = MlTranscript::new(MlHashFamily::Poseidon2);
        a.absorb(&[Goldilocks::from_u64(5)]);
        b.absorb(&[Goldilocks::from_u64(5)]);
        c.absorb(&[Goldilocks::from_u64(6)]);
        let (ca, cb, cc) = (a.challenge(), b.challenge(), c.challenge());
        assert_eq!(ca, cb);
        assert_ne!(ca, cc);
    }
}
