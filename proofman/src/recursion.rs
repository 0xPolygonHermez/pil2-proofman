...
pub fn generate_recursive_proof<F: PrimeField64>(
    ...
    let trace = std::mem::take(&mut witness.trace);
            ...
            if alive > 1 {
                ...
                for i in 0..alive.div_ceil(N_RECURSIVE_PROOFS_PER_AGGREGATION) {
                        ...
                        || alive <= N_RECURSIVE_PROOFS_PER_AGGREGATION
                    {
                        ...
                        let mut witness_proof = gen_witness_aggregation::<F>(
                            pctx,
                        ...
                        let recursive2_proof = gen_recursive_proof_size::<F>(pctx, setups, &witness_proof)?;
...
pub fn generate_recurser_aggregator_proof<F: PrimeField64>(
    ...
    let (trace, publics) = match generate_witness::<F>(setup, memory_handler_recursive_witness, 0, &zkin) {
        Ok(witness) => witness,
        Err(e) => {
...
fn generate_witness<F: PrimeField64>(
    ...
    let n_committed_pols = setup.n_cols;
