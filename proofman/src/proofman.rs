    ...
    GoldilocksQuinticExtension: ExtensionField<F>,
{
    ...
    fn register_prover_multiplicities(&self) -> ProofmanResult<()> {
        ...
        // Of the tables `fit_virtual_table_maps` left unfitted, the ones also in `range_ids` are
        // prover-owned through the range path above, not unclaimed; only the rest are genuinely
        // left to the std to count. The exact-map bytes are the one figure worth keeping here: they
        // are GPU-resident, per device.
