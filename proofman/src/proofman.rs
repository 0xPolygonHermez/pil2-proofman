...
use proofman_starks_lib_c::{gen_proof_c, get_map_totaln_c, commit_witness_c, calculate_hash_c};
...
use crate::{verify_basic_proofs, verify_constraints_proof, check_paths, print_summary_info, aggregate_proofs};

use proofman_common::{
    skip_prover_instance, StepsParams, ProofCtx, ProofType, ProofOptions, Prover, SetupCtx, SetupsVadcop,
};
...
impl<F: PrimeField + 'static> ProofMan<F> {
    ...
    pub fn generate_proof_constraints(
        ...
        Self::initialize_provers(pctx.clone(), sctx.clone());
    ...
    pub fn generate_proof(
        ...
        println!("{:?} PUBLICS AND PROOF VALUES {:?}", pctx.get_publics().clone(), proof_values_stage, );
        ...
        for instance_id in my_instances.iter() {
            ...
            air_instance.init_aux_trace(get_map_totaln_c(setup.p_setup.p_stark_info, false) as usize);
    ...
    fn initialize_provers(pctx: Arc<ProofCtx<F>>, sctx: Arc<SetupCtx<F>>) {
        ...
        for instance_id in my_instances.iter() {
            ...
            let air_instance = air_instances.get_mut(instance_id).unwrap();

            let n = 1 << setup.stark_info.stark_struct.n_bits;
            air_instance.init_aux_trace(get_map_totaln_c(setup.p_setup.p_stark_info, false) as usize);
            air_instance.init_evals(setup.stark_info.ev_map.len() * 3);
            air_instance.init_challenges((pctx.global_info.n_challenges.iter().sum::<usize>()) * 3);
            ...
            for commit_id in 0..n_custom_commits {
                ...
                if air_instance.custom_commits[commit_id].is_empty() {
                    air_instance.init_custom_commit(commit_id, n * n_cols);
                }
    ...
    fn hash_b_tree(values: Vec<Vec<F>>) -> Vec<F> {
        ...
        for i in (0..values.len() - 1).step_by(2) {
            ...
            if is_value1_zero && is_value2_zero {
                ...
                value = vec![F::zero(); 4];
                calculate_hash_c(value.as_mut_ptr() as *mut u8, buffer.as_mut_ptr() as *mut u8, 4);
                println!("HASHED VALUE {:?} OF {:?}", value, buffer);
            }
