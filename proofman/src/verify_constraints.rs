...
pub fn verify_constraints<F: Field>(
    ...
    let air_instance_id = pctx.dctx_find_air_instance_id(global_id);
    ...
    let (skip, constraints_skip) = skip_prover_instance(pctx.options.clone(), airgroup_id, air_id, air_instance_id);
