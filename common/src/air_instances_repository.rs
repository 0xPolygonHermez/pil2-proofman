...
impl<F: Field> AirInstancesRepository<F> {
    ...
    pub fn add_air_instance(&self, mut air_instance: AirInstance<F>, global_id: Option<usize>) -> usize {
        ...
        if global_id.is_some() {
            air_instance.global_id = global_id;
        } else {
            air_instance.global_id = Some(n_air_instances);
        }
        *instance_id += 1;
