...
///
/// Writes the verifier circom to `output_path`.
#[allow(clippy::too_many_arguments)]
pub fn run_pil2circom_js(
    starkinfo_path: &Path,
    verifierinfo_path: &Path,
    verkey_path: Option<&Path>,
    output_path: &Path,
    skip_main: bool,
    verkey_input: bool,
    enable_input: bool,
    input_challenges: bool,
) -> Result<()> {
    let root = resolve_stark_recurser_root().context("Cannot find stark-recurser. Run: npm install")?;
    let script = root.join("src/pil2circom/main_pil2circom.js");

    let mut cmd = std::process::Command::new("node");
    cmd.arg(&script).arg("-s").arg(starkinfo_path).arg("-i").arg(verifierinfo_path).arg("-o").arg(output_path);
    if let Some(vk) = verkey_path {
        cmd.arg("-v").arg(vk);
    }
    if skip_main {
        cmd.arg("--skipMain");
    ...
    let out = cmd
        ...
        .stderr(std::process::Stdio::inherit())
        .output()
        .context("Failed to execute node main_pil2circom.js")?;
    if !out.status.success() {
        bail!("pil2circom JS failed (exit {})", out.status.code().unwrap_or(-1));
    }
    Ok(())
}

/// Call `node src/main_gencircom.js` from the stark-recurser package.
///
/// `ejs_template` is a path relative to the stark-recurser root
/// (e.g. `"src/vadcop/templates/compressor.circom.ejs"`).
///
/// `basic_vk_paths` – paths to JSON files each containing `{"constRoot":[…]}`.
#[allow(clippy::too_many_arguments)]
pub fn run_gencircom_js(
    ejs_template: &str,
    starkinfo_paths: &[&Path],
    vadcop_path: Option<&Path>,
    verifier_filenames: &[&str],
    basic_vk_paths: &[&Path],
    agg_vk_paths: &[&Path],
    publics_paths: &[&Path],
    output_path: &Path,
    airgroup_id: Option<u64>,
    has_compressor: bool,
    has_recursion: bool,
) -> Result<()> {
    let root = resolve_stark_recurser_root().context("Cannot find stark-recurser. Run: npm install")?;
    let script = root.join("src/main_gencircom.js");
    let template_path = root.join(ejs_template);

    let mut cmd = std::process::Command::new("node");
    cmd.arg(&script).arg("-t").arg(&template_path).arg("-o").arg(output_path);
    for p in starkinfo_paths {
        cmd.arg("-s").arg(p);
    }
    for p in basic_vk_paths {
        cmd.arg("-b").arg(p);
    }
    for p in agg_vk_paths {
        cmd.arg("-a").arg(p);
    }
    for p in publics_paths {
        cmd.arg("-p").arg(p);
    }
    for name in verifier_filenames {
        cmd.arg("-n").arg(name);
    }
    if let Some(vp) = vadcop_path {
        cmd.arg("-v").arg(vp);
    }
    if let Some(id) = airgroup_id {
        cmd.arg("--airgroupId").arg(id.to_string());
    }
    if has_compressor {
        cmd.arg("--hasCompressor");
    }
    if has_recursion {
        cmd.arg("--hasRecursion");
    }
    ...
    let out = cmd
        ...
        .stderr(std::process::Stdio::inherit())
        .output()
        .context("Failed to execute node main_gencircom.js")?;
    if !out.status.success() {
        bail!("gencircom JS failed (exit {})", out.status.code().unwrap_or(-1));
    }
    Ok(())
}
