...
pub fn run_stats(opts: &StatsOptions) -> Result<()> {
    ...
    for (ag_idx, airgroup) in pilout.air_groups.iter().enumerate() {
        ...
        for (air_idx, air) in airgroup.airs.iter().enumerate() {
            ...
            // Hashes a verifier must perform for this air. Both the native and the in-circuit
            // verifier run the same algorithm, so this one count serves both; only the price of a
            // single hash differs between them.
            //
            // Only the family this setup is being built for. The rest of the geometry --
            // lastLevelVerification, grinding bits, the FRI steps -- comes from `stark_struct`,
            // which `generate_stark_struct` derived for `opts.hash` alone. Pairing another
            // family's arity with those would report a configuration no setup produces.
            {
                ...
                // The summary carries the headline number; the breakdown goes to the file.
                let summary = format!(
                    "{} | {} | {} | verifierHashes: {}",
                    airgroup_name,
                    air_name,
                ...
                stats_lines.push(format!(
                    "    total: {:>9} | leaf: {:>8} merkle: {:>8} fri: {:>8} transcript: {:>5} grinding: {} \
                     (arity {}, {} queries, llv {}, {} grinding bits)",
                    counts.total(),
                    counts.leaf,
                    counts.merkle,
                    counts.fri,
