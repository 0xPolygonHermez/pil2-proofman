...
use pil2_pilout::pilout::{
    self as pb, constraint, expression as expr_mod, global_expression as gexpr_mod, global_operand,
    hint_field, operand, SymbolType,
};
...
impl<'a> FormatCtx<'a> {
    ...
    fn try_unwrap_zero_rhs_inline(
        &mut self,
        operation: &expr_mod::Operation,
    ) -> Option<Expression> {
...
pub fn get_pilout_info(pilout: &pb::PilOut, airgroup_id: usize, air_id: usize) -> SetupResult {
    ...
    let n_constants = all_symbols
        .iter()
        .filter(|s| {
            s.sym_type == "fixed" && s.air_id == Some(air_id) && s.airgroup_id == Some(airgroup_id)
        })
        .count();
