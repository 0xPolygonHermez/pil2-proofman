// use crate::add_intermediate_pols::add_intermediate_polynomials;
// use crate::gen_pil_code::generate_pil_code;
...
//     let mut info_pil = prepare_pil(f, pil, stark_struct, pil2, &options);

//     let mut expressions = info_pil["expressions"].clone();
//     let constraints = info_pil["constraints"].clone();
//     let hints = info_pil["hints"].clone();
//     let mut symbols = info_pil["symbols"].clone();
//     let mut res = info_pil["res"].clone();
...
//         new_expressions = im_info["newExpressions"].clone();
...
//     map(&mut res, &mut symbols, &expressions, &constraints, &options);
...
//     let im_pols: Vec<&Value> =
//         res["cmPolsMap"].as_array().unwrap().iter().filter(|p| p["imPol"].as_bool().unwrap_or(false)).collect();

//     summary.push_str(&format!(
//         "| ImPols: {} => {} = {} + {} ",
//         im_pols.len(),
//         im_pols.iter().map(|p| p["dim"].as_u64().unwrap_or(0)).sum::<u64>(),
//         im_pols.iter().filter(|p| p["dim"] == json!(1)).map(|p| p["dim"].as_u64().unwrap_or(0)).sum::<u64>(),
//         im_pols.iter().filter(|p| p["dim"] == json!(3)).map(|p| p["dim"].as_u64().unwrap_or(0)).sum::<u64>(),
//     ));

//     println!("------------------------------------------------------------");
//     println!("SUMMARY | {} | {}", pil["name"], summary);
//     println!("------------------------------------------------------------");

//     let stats = json!({"summary": summary, "intermediatePolynomials": res["imPolsInfo"]});

//     res.as_object_mut().unwrap().remove("nCommitments");
//     res.as_object_mut().unwrap().remove("imPolsStages");
//     res.as_object_mut().unwrap().remove("pilPower");
//     res.as_object_mut().unwrap().remove("imPolsInfo");
