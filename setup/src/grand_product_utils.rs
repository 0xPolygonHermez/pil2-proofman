...
//     airgroup_id: u64,
//     air_id: u64,
//     f: &str, // Placeholder for the field element arithmetic type
// ) {
//     let stage = 2;
//     let dim = 3;
...
//             // Compute ks values
//             let ks = get_ks(f, ci["pols"].as_array().unwrap().len() - 1);
...
//                             e.mul(e.mul(delta.clone(), e.number(ks[j - 1] as f64)), e.x()),
//                         ),
//                         gamma.clone(),
//                     ),
//                 );
...
//                     e.add(
//                         e.add(
//                             e.exp(pol.as_u64().unwrap() as usize),
...
//                 let num_dim = get_exp_dim(&expressions_array, ci_ctx["numId"].as_u64().unwrap() as usize);

//                 ci_ctx.insert("denId", json!(expressions_array.len()));
//                 expressions_array.push(json!(den_exp));
//                 let den_dim = get_exp_dim(&expressions_array, ci_ctx["denId"].as_u64().unwrap() as usize);
//             }
...
//             let c1_id = expressions_array.len();
...
//             );
//             let c2_id = expressions_array.len();
