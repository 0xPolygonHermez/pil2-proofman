...
pub fn compressor(r1cs: &R1csFile, options: &PlonkOptions) -> SetupResult {
    ...
    // CustPoseidon1 (compression / Davies-Meyer) gates come first, then Poseidon1 (sponge) gates,
    ...
    let process_poseidon1 = |s: &[u64],
        ...
        let expected = POSEIDON_WIDTH + key_off + 11 * POSEIDON_WIDTH + POSEIDON_WIDTH;
        ...
        let r3 = &s[im_base + 2 * POSEIDON_WIDTH..im_base + 3 * POSEIDON_WIDTH]; // im[2]
                                                                                 // im[3] = R4 (post-P state) — not stored in the 9-row variant
        let im1 = &s[im_base + 4 * POSEIDON_WIDTH..im_base + 5 * POSEIDON_WIDTH]; // im[4]: h1 anchors[0..10]
                                                                                  // im[5] = midState (intermediate, not used with single-chain layout)
        ...
        for i in 0..POSEIDON_WIDTH {
            ...
            s_map[i + COL_P][r] = initial_st[i] as u32; // row 0 chain slot = R0 (permuted input for Cust)
