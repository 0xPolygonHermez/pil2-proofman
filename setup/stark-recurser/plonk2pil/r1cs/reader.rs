...
impl fmt::Display for R1CSParsingError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            R1CSParsingError::SectionNotPresent(section) => {
                write!(f, "Section \"{}\" not present", section)
            }
        }
    }
}
...
fn read_section(reader: &mut BufReader<File>, header: &[u8]) -> Result<u64, std::io::Error> {
    ...
    reader.read_exact(&mut buffer)?;
    if buffer != header {
        return Err(std::io::Error::other(R1CSParsingError::InvalidSectionHeader.to_string()));
    }
    let go_back = reader.stream_position()?;
...
fn end_section(reader: &mut BufReader<File>, go_back: u64) -> Result<usize, std::io::Error> {
    let go_back_1 = reader.stream_position()?;
...
type TripleHashMap<T> = (HashMap<T, BigInt>, HashMap<T, BigInt>, HashMap<T, BigInt>);

fn read_constraint<T>(reader: &mut BufReader<File>, field_size: usize) -> Result<TripleHashMap<T>, std::io::Error>
where
    T: AsRef<[u8]> + std::cmp::Ord + std::hash::Hash + From<Vec<u8>>,
{
...
impl R1CSReader {
    ...
    pub fn start_custom_gates_applied_section(
        mut r1cs: R1CSReader,
    ) -> Result<CustomGatesAppliedSection, std::io::Error> {
...
pub fn read_r1cs(input: &str) -> Result<R1CSData, std::io::Error> {
    ...
    for _ in 0..n_sections {
        ...
        if section_type >= 1 && section_type <= n_sections as u8 {
            ...
            return Err(std::io::Error::other(R1CSParsingError::InvalidSectionType.to_string()));
        }
...
fn read_initialization(r1cs: &mut R1CSReader) -> Result<Vec<u8>, std::io::Error> {
    ...
    r1cs.reader.read_exact(&mut buffer)?;
    if buffer != MAGIC {
        return Err(std::io::Error::other(R1CSParsingError::InvalidMagicNumber.to_string()));
    }
    let mut buffer = vec![0; VERSION.len()];
    r1cs.reader.read_exact(&mut buffer)?;
    if buffer != VERSION {
        return Err(std::io::Error::other(R1CSParsingError::InvalidVersion.to_string()));
    }
    let mut buffer = vec![0; 4];
    r1cs.reader.read_exact(&mut buffer)?;
    Ok(buffer)
...
fn read_sections(info: &mut R1CSData, mut r1cs: R1CSReader, starts: HashMap<u8, u64>) -> Result<(), std::io::Error> {
    ...
    if let Some(&start) = starts.get(&1) {
        ...
        return Err(std::io::Error::other(R1CSParsingError::SectionNotPresent("Header".to_string()).to_string()));
    }
    ...
    if let Some(&start) = starts.get(&2) {
        ...
        return Err(std::io::Error::other(R1CSParsingError::SectionNotPresent("Constraints".to_string()).to_string()));
    }
    ...
    if let Some(&start) = starts.get(&3) {
        ...
        return Err(std::io::Error::other(R1CSParsingError::SectionNotPresent("Signals".to_string()).to_string()));
    }
