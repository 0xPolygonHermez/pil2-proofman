...
//!
//! Entry points:
//! - [`gen_circom_circuit`] — replaces `run_gencircom_js`
//! - [`gen_solidity`]       — replaces `run_gen_solidity` node inline script
...
pub fn gen_circom_circuit(input: &GenCircomCircuitInput<'_>) -> Result<String> {
    ...
    match input.template_name {
        ...
        other => {
            bail!("gen_circom_circuit: template '{}' is not yet implemented in Rust (use run_gencircom_js)", other)
        }
    }
}
