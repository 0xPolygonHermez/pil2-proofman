...
template VerifyMerkleHashUntilLevelEmpty(eSize, elementsInLinear, arity, nLastLevels, height) {
    ...
    signal calculatedVal[4] <== LinearHash(elementsInLinear, eSize)(values);
...
template CalculateLevelMT(nLevels, arity, num_nodes_level) {
    ...
    if (nLevels == 0) {
        ...
        for (var j = 0; j < next_n; j++) {
            // CustPoseidon with key=[0,0] computes the same hash as the legacy
            // sponge-mode Poseidon1_16 (no input reordering when both bits are 0),
            // and matches the PIL CustPoseidon1_16 gate used for merkle compression.
            hashes[j] = CustPoseidon(4, 4);
