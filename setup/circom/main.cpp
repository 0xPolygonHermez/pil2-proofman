    ...
    uint64_t N, uint64_t nPublics, uint64_t nCommitedPols, uint64_t nMutexes, void* pSignalValues)
{
    ...
    // Scatter into the committed-pol trace, parallelized over contiguous ROW blocks —
    // never over j, which would put several threads on the same 64-byte trace line.
    // Everything read here (signalValues, adds_ext, p_sMap) is immutable, so the blocks
