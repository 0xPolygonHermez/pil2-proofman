...
use crate::{Lookup2_15Trace, LOOKUP_2_15_AIR_IDS, LOOKUP_AIRGROUP_ID};
    ...
    Standard: Distribution<F>,
{
    ...
    pub fn execute(&self, pctx: Arc<ProofCtx<F>>, ectx: Arc<ExecutionCtx>, sctx: Arc<SetupCtx>) {
        let num_rows = pctx.global_info.airs[LOOKUP_AIRGROUP_ID][LOOKUP_2_15_AIR_IDS[0]].num_rows;
        let trace = Lookup2_15Trace::new(num_rows);

        let air_instance = AirInstance::new(sctx.clone(), LOOKUP_AIRGROUP_ID, LOOKUP_2_15_AIR_IDS[0], None, trace.buffer.unwrap());

        let (is_myne, gid) = ectx.dctx.write().unwrap().add_instance(LOOKUP_AIRGROUP_ID, LOOKUP_2_15_AIR_IDS[0], 1);
        if is_myne {
    ...
    Standard: Distribution<F>,
{
    fn calculate_witness(
        ...
        if stage == 1 {
            ...
            let mut trace = Lookup2_15Trace::map_buffer(buffer.as_mut_slice(), num_rows, 0).unwrap();
