...
pub struct DirectUpdateSumLocal {
    instance_ids: RwLock<Vec<usize>>,
}

impl DirectUpdateSumLocal {
    ...
    Standard: Distribution<F>,
{
    fn execute(&self, pctx: Arc<ProofCtx<F>>) -> Vec<usize> {
        let global_ids = vec![pctx
            .add_instance(DirectUpdateSumLocalTrace::<usize>::AIRGROUP_ID, DirectUpdateSumLocalTrace::<usize>::AIR_ID)];
