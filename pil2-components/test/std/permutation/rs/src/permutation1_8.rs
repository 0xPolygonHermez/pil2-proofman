    ...
    Standard: Distribution<F>,
{
    ...
    pub fn new(wcm: Arc<WitnessManager<F>>) -> Arc<Self> {
        ...
        wcm.register_component(permutation1_8.clone(), Some(PERMUTATION_AIRGROUP_ID), Some(PERMUTATION_1_8_AIR_IDS));
    ...
    pub fn execute(&self, pctx: Arc<ProofCtx<F>>, ectx: Arc<ExecutionCtx>, sctx: Arc<SetupCtx>) {
        ...
        let air_instance =
            AirInstance::new(sctx.clone(), PERMUTATION_AIRGROUP_ID, PERMUTATION_1_8_AIR_IDS[0], None, trace.buffer.unwrap());
        let (is_myne, gid) =
            ectx.dctx.write().unwrap().add_instance(PERMUTATION_AIRGROUP_ID, PERMUTATION_1_8_AIR_IDS[0], 1);
        if is_myne {
    ...
    Standard: Distribution<F>,
{
    fn calculate_witness(
        ...
        if stage == 1 {
            ...
            let mut trace =
                Permutation1_8Trace::map_buffer(buffer.as_mut_slice(), num_rows, 0).unwrap();
