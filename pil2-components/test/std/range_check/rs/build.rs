...
fn main() {
    ...
    let pil_file = root_path.join("test/std/range_check/build.pil");
    let build_dir = root_path.join("test/std/range_check/build");
    let pilout_file = build_dir.join("range_check.pilout");
    let pil_helpers_dir = root_path.join("test/std/range_check/src/pil_helpers");
