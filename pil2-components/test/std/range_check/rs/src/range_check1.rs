    ...
    Standard: Distribution<F>,
{
    ...
    pub fn execute(&self, pctx: Arc<ProofCtx<F>>, ectx: Arc<ExecutionCtx>, sctx: Arc<SetupCtx>) {
        let num_rows = pctx.global_info.airs[RANGE_CHECK_1_AIRGROUP_ID][RANGE_CHECK_1_AIR_IDS[0]].num_rows;
        let trace = RangeCheck1Trace::new(num_rows);

        let air_instance =
            AirInstance::new(sctx.clone(), RANGE_CHECK_1_AIRGROUP_ID, RANGE_CHECK_1_AIR_IDS[0], None, trace.buffer.unwrap());
        let (is_myne, gid) =
    ...
    Standard: Distribution<F>,
{
    fn calculate_witness(
        ...
        log::debug!("{}: ··· Witness computation for AIR '{}' at stage {}", Self::MY_NAME, "RangeCheck1", stage);
            ...
            let mut trace = RangeCheck1Trace::map_buffer(buffer.as_mut_slice(), num_rows, 0).unwrap();
