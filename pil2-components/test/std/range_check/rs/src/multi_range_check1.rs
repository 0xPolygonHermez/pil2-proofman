    ...
    Standard: Distribution<F>,
{
    ...
    pub fn new(wcm: Arc<WitnessManager<F>>, std_lib: Arc<Std<F>>) -> Arc<Self> {
        ...
        let airgroup_id = MultiRangeCheck1Trace::<F>::AIRGROUP_ID;
        let air_id = MultiRangeCheck1Trace::<F>::AIR_ID;

        wcm.register_component(multi_range_check1.clone(), airgroup_id, air_id);
    ...
    pub fn execute(&self, pctx: Arc<ProofCtx<F>>) {
        ...
        let mut trace = MultiRangeCheck1Trace::new_zeroes();
...
impl<F: PrimeField> WitnessComponent<F> for MultiRangeCheck1<F> where Standard: Distribution<F> {}
