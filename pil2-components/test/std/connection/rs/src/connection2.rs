...
use crate::{Connection21Trace, CONNECTION_2_AIR_IDS, CONNECTION_12_AIRGROUP_ID};
    ...
    Standard: Distribution<F>,
{
    ...
    pub fn new(wcm: Arc<WitnessManager<F>>) -> Arc<Self> {
        ...
        wcm.register_component(connection2.clone(), Some(CONNECTION_12_AIRGROUP_ID), Some(CONNECTION_2_AIR_IDS));
    ...
    pub fn execute(&self, pctx: Arc<ProofCtx<F>>, ectx: Arc<ExecutionCtx>, sctx: Arc<SetupCtx>) {
        ...
        let (buffer_size, _) = ectx
            .buffer_allocator
            .as_ref()
            .get_buffer_info(&sctx, CONNECTION_12_AIRGROUP_ID, CONNECTION_2_AIR_IDS[0])
            .unwrap();

        let buffer = vec![F::zero(); buffer_size as usize];

        let air_instance = AirInstance::new(CONNECTION_12_AIRGROUP_ID, CONNECTION_2_AIR_IDS[0], None, buffer);
    ...
    Standard: Distribution<F>,
{
    fn calculate_witness(
        ...
        if stage == 1 {
            ...
            let num_rows = pctx.pilout.get_air(CONNECTION_12_AIRGROUP_ID, CONNECTION_2_AIR_IDS[0]).num_rows();
            let mut trace =
