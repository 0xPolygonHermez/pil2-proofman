...
const BYTE: usize = 1 << 8;

pub struct U8AirExtended<F: Field> {
...
impl<F: PrimeField> U8AirExtended<F> {
    const MY_NAME: &'static str = "U8AirExt";
    ...
    pub fn new(wcm: Arc<WitnessManager<F>>) -> Arc<Self> {
        ...
        let hint_global = get_hint_ids_by_name(sctx.get_global_bin(), "u8airext");
        ...
        let u8air_extended = Arc::new(Self {
            wcm: wcm.clone(),
        ...
        wcm.register_component(u8air_extended.clone(), Some(airgroup_id), Some(&[air_id]));
...
impl<F: PrimeField> WitnessComponent<F> for U8AirExtended<F> {
    fn start_proof(&self, pctx: Arc<ProofCtx<F>>, ectx: Arc<ExecutionCtx<F>>, sctx: Arc<SetupCtx<F>>) {
        ...
        let u8air_extended_hints = get_hint_ids_by_name(setup.p_setup.p_expressions_bin, "u8airext");
        if !u8air_extended_hints.is_empty() {
            self.hint.store(u8air_extended_hints[0], Ordering::Release);
        }
        ...
        *self.mul_column.lock().unwrap() = get_hint_field::<F>(
            ...
            &pctx,
            &mut air_instance,
            u8air_extended_hints[0] as usize,
            "reference",
