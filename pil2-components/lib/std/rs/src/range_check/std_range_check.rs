...
impl<F: PrimeField> StdRangeCheck<F> {
    ...
    pub fn new(mode: StdMode, wcm: Arc<WitnessManager<F>>) -> Arc<Self> {
        ...
        let u8air = create_air::<U8Air<F>, F>(sctx.clone(), &wcm, &u8air_hint);
        let u16air = create_air::<U16Air<F>, F>(sctx.clone(), &wcm, &u16air_hint);
        let specified_ranges = create_air::<SpecifiedRanges<F>, F>(sctx.clone(), &wcm, &specified_ranges_hint);
            ...
            T: AirComponent<F>,
        {
            ...
            Some(T::new(wcm.clone(), None, Some(airgroup_id), Some(air_id)))
        }
    }
