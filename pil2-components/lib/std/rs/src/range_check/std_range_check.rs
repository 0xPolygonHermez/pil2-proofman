...
impl<F: PrimeField> StdRangeCheck<F> {
    ...
    pub fn new(pctx: Arc<ProofCtx<F>>, sctx: &SetupCtx<F>) -> Arc<Self> {
        ...
        let u8air = create_air::<U8Air<F>, F>(&pctx, sctx, &u8air_hint);
        let u16air = create_air::<U16Air<F>, F>(&pctx, sctx, &u16air_hint);
        let specified_ranges = create_air::<SpecifiedRanges<F>, F>(&pctx, sctx, &specified_ranges_hint);
        ...
        if let Some(std_rc_users) = get_hint_ids_by_name(sctx.get_global_bin(), "std_rc_users").first() {
            let num_users: usize = get_global_hint_field_constant_as(sctx, *std_rc_users, "num_users");
            ...
            for i in 0..num_users {
                ...
                Self::register_ranges(sctx, airgroup_id, air_id, &mut ranges);
            }
        }

        return Arc::new(Self { pctx, ranges, u8air, u16air, specified_ranges });

        // Helper function to instantiate AIRs
        fn create_air<T, F: PrimeField>(pctx: &ProofCtx<F>, sctx: &SetupCtx<F>, hints: &[u64]) -> Option<Arc<T>>
        where
            T: AirComponent<F>,
        {
            if hints.is_empty() {
            ...
            let airgroup_id: usize = get_global_hint_field_constant_as(sctx, hints[0], "airgroup_id");
            let air_id: usize = get_global_hint_field_constant_as(sctx, hints[0], "air_id");
