...
/// Every virtual range table as (table id, bias); a lookup addresses row `value + bias`
/// (bias = -min for specified ranges, 0 otherwise). Registration is left to the host binary.
pub fn collect_prover_owned_ranges<F: PrimeField64>(
    ...
    for i in 0..num_users {
        ...
        for hint in get_hint_ids_by_name(setup.p_setup.p_expressions_bin, "range_def") {
            ...
            // A non-virtual range has a table air of its own, which the std keeps counting.
