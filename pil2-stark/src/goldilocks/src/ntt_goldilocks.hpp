...
public:
    ...
    void computeQ_inplace(Goldilocks::Element *d_tree, uint64_t offset_cmQ, uint64_t offset_q, uint64_t qDeg, uint64_t qDim, Goldilocks::Element shiftIn, uint64_t N, uint64_t NExtended, uint64_t nCols, DeviceCommitBuffers *d_buffers, uint64_t offset_helper, double *nttTime=nullptr, double *merkleTime=nullptr);
