...
__global__ void br_ntt_8_steps_blocks(gl64_t *data, gl64_t *twiddles, gl64_t* d_r, uint32_t domain_size, uint32_t log_domain_size, uint32_t ncols, uint32_t base_step, bool suffle, bool inverse, bool extend, uint64_t maxLogDomainSize);
...
__global__ void br_ntt_group_blocks(gl64_t *data, gl64_t *twiddles, gl64_t* d_r, uint32_t stage, uint32_t domain_size, uint32_t log_domain_size, uint32_t ncols, bool inverse, bool extend, uint64_t maxLogDomainSize)
{
    uint32_t i = blockIdx.x;
    uint32_t col = threadIdx.x;
    ...
    gl64_t* data_block = data + block_idx * (domain_size << 3); //4 cols per block and is embede in the Nextended size
    ...
    if (i < domain_size / 2 && col < ncols)
    {
        ...
        uint32_t index2 = index1 + half_group_size;
        gl64_t factor = twiddles[group_pos * ((1 << maxLogDomainSize) >> (stage + 1))];  // Use actual domain size
        gl64_t odd_sub = gl64_t((uint64_t)data_block[index2 * ncols_block + col]) * factor;
        gl64_t result1 = gl64_t((uint64_t)data_block[index1 * ncols_block + col]) + odd_sub;
        gl64_t result2 = gl64_t((uint64_t)data_block[index1 * ncols_block + col]) - odd_sub;
        ...
        data_block[index1 * ncols_block + col] = result1;
        data_block[index2 * ncols_block + col] = result2;
    }
}
...
__global__ void br_ntt_8_steps_blocks(gl64_t *data, gl64_t *twiddles, gl64_t* d_r, uint32_t domain_size, uint32_t log_domain_size, uint32_t ncols, uint32_t base_step, bool suffle, bool inverse, bool extend, uint64_t maxLogDomainSize)
{
    ...
    uint32_t offset = domain_size << 3; //4 cols per block and is embede in the Nextended size
       ...
       for(int i=0; i<ncols_block; i++){
        ...
        for(int loc_step=0; loc_step<n_loc_steps; loc_step++){
            ...
            if (threadIdx.x < 128){ // Only process first 128 threads (half of them)
                ...
                index1 = index1 * ncols_block;
                index2 = index2 * ncols_block;
        ...
        if(inverse && (base_step + n_loc_steps) >= log_domain_size){
           ...
           for(int i=0; i<ncols_block; i++){
                data_block[row*+i] = tile[threadIdx.x*4+i];
           }
        }
    }   
}
...
__global__ void reverse_permutation_blocks(gl64_t *data, uint32_t log2_domain_size_in, uint64_t domain_size_out, uint32_t ncols)
{
    ...
    assert(domain_size_out == 2*domain_size_in);
