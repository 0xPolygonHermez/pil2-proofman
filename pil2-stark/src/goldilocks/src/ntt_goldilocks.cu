...
void NTT_Goldilocks_GPU::LDE_MerkleTree_GPU_inplace(Goldilocks::Element *d_tree, gl64_t *d_dst_ntt, uint64_t offset_dst_ntt, gl64_t *d_src_ntt, uint64_t offset_src_ntt, u_int64_t n_bits, u_int64_t n_bits_ext, u_int64_t ncols, TimerGPU &timer, cudaStream_t stream)
{
    ...
    ntt_cuda_blocks(d_aux, d_r, d_fwd_twiddle_factors, d_inv_twiddle_factors, n_bits, n_bits_ext, ncols, true, true, stream, maxLogDomainSize, d_src_ntt);
    ...
    ntt_cuda_blocks(d_aux, d_r, d_fwd_twiddle_factors, d_inv_twiddle_factors, n_bits_ext, n_bits_ext, ncols, false, false, stream, maxLogDomainSize, d_src_ntt);
    ...
    dim3 grid((size + block.x - 1) / block.x,
             (ncols + block.y - 1) / block.y);
    size_t sharedMemSize = block.x * block.y * sizeof(gl64_t);
    prepareBlocksInputRowMajor<<<grid, block, sharedMemSize, stream>>>(d_dst_ntt, d_src_ntt_, size, ncols);        
    ntt_cuda_blocks(d_dst_ntt, d_r, d_fwd_twiddle_factors, d_inv_twiddle_factors, n_bits, n_bits_ext, ncols, true, true, stream, maxLogDomainSize, d_src_ntt); 
    ntt_cuda_blocks(d_dst_ntt, d_r, d_fwd_twiddle_factors, d_inv_twiddle_factors, n_bits_ext, n_bits_ext, ncols, false, false, stream, maxLogDomainSize, d_src_ntt);
