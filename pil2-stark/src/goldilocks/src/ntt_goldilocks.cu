    ...
    void step(int iterations)
    {
        ...
        if (dit) {
            ...
            if (num_blocks < (uint32_t)Z) {
                ...
                if (fuseq)
                    nttDitColMajorKernel<4, false, false, true><<<dim3(num_blocks/Z, ncols), block_size, (Z+1)*shared_sz, s>>>(NTT_ARGS_Q, cosetSrc, nullptr, lgBlowup, cosetStride);
                else if (fuse)
                    nttDitColMajorKernel<4, false, true><<<dim3(num_blocks/Z, ncols), block_size, (Z+1)*shared_sz, s>>>(NTT_ARGS, cosetSrc, cosetPows, lgBlowup, cosetStride);
                else
                    nttDitColMajorKernel<4><<<dim3(num_blocks/Z, ncols), block_size, (Z+1)*shared_sz, s>>>(NTT_ARGS, nullptr, nullptr, 0, 0);
            } else {
                nttDitColMajorKernel<4, true><<<dim3(num_blocks/Z, ncols), block_size, (Z+1)*shared_sz, s>>>(NTT_ARGS, nullptr, nullptr, 0, 0);
            }
            stage += iterations;
        } else {
            if (num_blocks < (uint32_t)Z)
                ...
                nttDifColMajorKernel<4><<<dim3(num_blocks/Z, ncols), block_size, (Z+1)*shared_sz, s>>>(NTT_ARGS);
            else
                nttDifColMajorKernel<4, true><<<dim3(num_blocks/Z, ncols), block_size, (Z+1)*shared_sz, s>>>(NTT_ARGS);
            stage -= iterations;
        }
        #undef NTT_ARGS
        CHECKCUDAERR(cudaGetLastError());
