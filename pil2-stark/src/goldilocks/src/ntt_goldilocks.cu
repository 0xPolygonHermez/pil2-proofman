...
__global__ void br_ntt_8_steps(gl64_t *data, gl64_t *twiddles, uint32_t domain_size, uint32_t log_domain_size, uint32_t ncols, uint32_t base_step, bool suffle, uint64_t maxLogDomainSize);
__global__ void br_ntt_group(gl64_t *data, gl64_t *twiddles, uint32_t i, uint32_t domain_size, uint32_t ncols, uint64_t maxLogDomainSize);
...
__global__ void br_ntt_group(gl64_t *data, gl64_t *twiddles, uint32_t stage, uint32_t domain_size, uint32_t ncols, uint64_t maxLogDomainSize)
{
...
{   
    ...
    gl64_t *ptr_twiddles = nullptr;
    if (inverse)
    {
        ptr_twiddles = d_inv_twiddle_factors[device_id];
    }else{
        ptr_twiddles = d_fwd_twiddle_factors[device_id];
    }

    if(log_domain_size >= 8) {
         for(uint32_t step = 0; step < log_domain_size; step+=8){
            br_ntt_8_steps<<<domain_size / 256, 256, 0, stream>>>(data, ptr_twiddles, domain_size, log_domain_size, ncols, step, true, maxLogDomainSize);
            CHECKCUDAERR(cudaGetLastError());               
        }
    } else {
        for (uint32_t i = 0; i < log_domain_size; i++)
        {
            br_ntt_group<<<domain_size / 2, ncols, 0, stream>>>(data, ptr_twiddles, i, domain_size, ncols, maxLogDomainSize);
            CHECKCUDAERR(cudaGetLastError());
        }
    }
