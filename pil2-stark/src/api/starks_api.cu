...
void *gen_recursive_proof(void *pSetupCtx_, char* globalInfoFile, uint64_t airgroupId, void* witness, void *pConstPols, void *pConstTree, void* pPublicInputs, char* proof_file, bool vadcop, void* d_buffers_) {
    ...
    CHECKCUDAERR(cudaMemcpy(d_buffers->d_witness, witness, sizeWitness, cudaMemcpyHostToDevice));
    ...
    time = omp_get_wtime();
    void * proof = genRecursiveProof_gpu<Goldilocks::Element>(*setupCtx, globalInfo, airgroupId, (Goldilocks::Element *)witness,(Goldilocks::Element *)pConstPols, (Goldilocks::Element *)pConstTree, (Goldilocks::Element *)pPublicInputs, string(proof_file), d_buffers);
    time = omp_get_wtime() - time;
...
void* gen_device_commit_buffers(void * maxSizes_) {
    ...
    CHECKCUDAERR(cudaMalloc(&buffers->d_witness, maxSizes->maxN * maxSizes->maxWitPols * sizeof(Goldilocks::Element)));
