// Polynomial<Engine> tests, ported from the pil-fflonk repository
// (0xPolygonHermez/pilfflonk, test/polynomial.test.cpp) and run here against
// rapidsnark's Polynomial, which is the implementation pil2 keeps.
...
// pil-fflonk had a Polynomial::fromCoefficients(E, buf, len) factory. It is NOT
// reachable through pil2's existing API: the (E, reservedBuffer, length)
// constructor runs initialize(), which parsets the buffer to zero even when it
// does not own it — so wrapping a populated buffer destroys it. Kept as a test
// helper; promote it to the class only if pil2-fflonk needs it.
static Polynomial<AltBn128::Engine>* fromCoefficients(AltBn128::FrElement* coefficients, u_int64_t length) {
    auto pol = new Polynomial<AltBn128::Engine>(E, length);
    for (u_int64_t i = 0; i < length; i++) {
        pol->coef[i] = coefficients[i];
    }
    pol->fixDegree();
    return pol;
}
...
TEST(POLYNOMIAL, isEqual) {
    ...
    auto polA = fromCoefficients(buffer, 8);
    auto polB = fromCoefficients(buffer, 8);
...
TEST(POLYNOMIAL, byXSubValue) {
    ...
    auto polA = fromCoefficients(bufferA, 8);
    auto polB = fromCoefficients(bufferB, 8);
...
TEST(POLYNOMIAL, byXNSubValue) {
    ...
    delete polA;
    polA = fromCoefficients(bufferA, 8);
    auto polC = fromCoefficients(bufferC, 8);
...
// pil-fflonk carried a dedicated divByXSubValue(v); pil2 already covers it with
// divByMonic(1, v) and divByZerofier(1, v), both of which divide by (X^n - beta)
// with n = 1. This exercises the two existing entry points on pil-fflonk's
// vector rather than reintroducing a third spelling of the same division.
TEST(POLYNOMIAL, divByXSubValue) {
    ...
    auto expected = fromCoefficients(bufferA, 8);

    auto viaMonic = fromCoefficients(bufferB, 8);
    viaMonic->divByMonic(1, element);
    ASSERT_TRUE(expected->isEqual(*viaMonic));

    auto viaZerofier = fromCoefficients(bufferB, 8);
    viaZerofier->divByZerofier(1, element);
    ASSERT_TRUE(expected->isEqual(*viaZerofier));
...
TEST(POLYNOMIAL, divZh) {
    ...
    auto polA = fromCoefficients(bufferA, 8);
    auto polB = fromCoefficients(bufferB, 4);
