...
#pragma omp parallel for schedule(static, chunk1)
    ...
    if (a != dst_)
    {
        ...
        // Only reachable with size <= 1 (the assert above covers the rest), so a
        // plain memcpy is enough. Note the units: Goldilocks::parcpy, which this
        // was ported from, counts elements, while ThreadUtils::parcpy counts
        // bytes -- passing `size * ncols` to the latter was 32x short.
        memcpy(dst_, a, size * ncols * sizeof(AltBn128::FrElement));
    }
}
