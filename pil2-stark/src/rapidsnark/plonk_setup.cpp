...
void PlonkSetup::computeFFConstraints(BinFile &r1cs, R1csHeader &r1csHeader) {
    ...
    for (uint64_t i = 0; i < settings.nPublics; i++) {
        plonkConstraints.push_back(R1csConstraintProcessor::getPlonkConstantConstraint(E, i + 1));
    }
...
FrElement PlonkSetup::computeW3() {
...
FrElement PlonkSetup::computeW4() {
    mpz_t result;
...
FrElement PlonkSetup::computeW8() {
    mpz_t result;
...
FrElement PlonkSetup::getOmegaCubicRoot() {
...
void PlonkSetup::writeZkeyFile(string &zkeyFilename, BinFile &fdPtau) {
    ...
    ss << "··· Writing Section " << Zkey::ZKEY_FF_HEADER_SECTION << ". Plonk Header";
    LOG_INFO(ss);
    ss.str("");
...
void PlonkSetup::writeC0(BinFileWriter &zkeyFile) {
...
G1Point PlonkSetup::multiExponentiation(Polynomial<AltBn128::Engine> *polynomial, u_int32_t nx, u_int64_t x[]) {
    G1Point value;
