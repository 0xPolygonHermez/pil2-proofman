...
struct MulJobDev {
    // Hot first, deliberately. Every thread walks EVERY job of the air on every row -- 1.03 billion
    // job-iterations for Keccakf -- and reads these ten fields to decide what to do. With the
    // polynomials in front they sat past offset 4900 of an 18 KB struct, so one job's control flow
    // touched ~10 scattered cache lines; grouped here it is one or two.
