...
public:
    ...
    ExpressionsGPU(SetupCtx &setupCtx, uint32_t nParamsMax, uint64_t nrowsPack_ = 64, uint32_t nBlocks_ = 256) : ExpressionsCtx(setupCtx), nParamsMax(nParamsMax), nrowsPack(nrowsPack_), nBlocks(nBlocks_)
    {
        ...
        {
            cudaMalloc(&deviceArgs.bufferT_[i], bufferSize * sizeof(Goldilocks::Element));
            contMem += bufferSize * sizeof(Goldilocks::Element);
        }
...
__global__ void computeExpressions_(DeviceArguments *d_deviceArgs)
{
    ...
    while (chunk_idx < nchunks)
    {
        ...
        for (uint64_t j = 0; j < nDests; ++j)
        {
            ...
            if (dests[j].nParams == 2)
            {
                // rick: this has not been tested with fibonacci example
