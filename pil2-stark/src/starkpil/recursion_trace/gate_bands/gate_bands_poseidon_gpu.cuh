#ifndef GATE_BANDS_POSEIDON_GPU_CUH
#define GATE_BANDS_POSEIDON_GPU_CUH

// Device back-end for the Poseidon band kinds. One thread per band: reads the boundary cells out
// of the device trace, runs its family's width-16 permutation, and scatters the snapshots into the
// interior.
//
// This is a SECOND implementation of the permutation, not the host one recompiled -- deliberately,
// and it must stay that way. BLAKE3 shares one templated implementation because its constants are
// ~100 bytes (a function-local constexpr materialises on both sides) and because host and device
// want the same shape. Neither holds here: Poseidon's tables are ~11 KB, which belongs in
// __constant__ and would be ruinous as a function-local constexpr in device code, and this version
// STREAMS snapshots through a sink to keep a thread's working set to the 16-word state -- buffering
// all 12 groups the way the host does costs 1.5 KB of local memory per thread and about a third of
// the kernel's throughput. GateBandsGPU.MatchesTheHostExpander pins the two together cell for cell.
//
// Included by exactly one translation unit: the __constant__ tables below are per-TU under
// whole-program compilation, so a second includer would get its own uninitialised copy.
...
__global__ void expandPoseidonBandsKernel(uint64_t *trace, uint64_t nCols,
                                          const uint64_t *bands, uint64_t nBands) {
...
// This family needs no dense scratch; the uniform launcher signature still carries one.
constexpr uint64_t SCRATCH_WORDS = 0;

// Uploads the constant tables to the current device. Per-device state, so the caller selects the
// device first. Idempotent, but it must not race work already reading the tables on that device.
inline void upload_constants() {
...
// `nRows`, `aux` and the scratch are the uniform back-end signature; this family reads none of
// them. Row bounds are enforced host-side by first_bad_band, once, so no kernel guard is needed.
