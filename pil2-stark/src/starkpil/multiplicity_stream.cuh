...
inline std::map<std::pair<int, uint64_t>, std::pair<uint64_t*, size_t>>& mulStreamTiles() {
    static std::map<std::pair<int, uint64_t>, std::pair<uint64_t*, size_t>> m;
    return m;
}
...
inline uint64_t* mulStreamTile(int gpuId, uint64_t slotIdx, size_t elems) {
    ...
    auto& e = mulStreamTiles()[{gpuId, slotIdx}];
...
inline void mulStreamHook(const uint64_t *dPacked, const uint64_t *dWidths,
    ...
    uint64_t *tile = mulStreamTile(gpuId, c->slotIdx, dims.nCols * tileRows);
    if (tile == nullptr) {
