...
void genProof_gpu(SetupCtx& setupCtx, gl64_t *d_aux_trace, gl64_t *d_const_pols, gl64_t *d_const_tree, StepsParams *params_pinned, Goldilocks::Element *proof_buffer_pinned, TranscriptGL_GPU *d_transcript, TranscriptGL_GPU *d_transcript_helper, AirInstanceInfo *air_instance_info, StepsParams *d_params, bool skipRecalculation, TimerGPU &timer, cudaStream_t stream) {
    ...
    *params_pinned = h_params;
    ...
    calculateWitnessSTD_gpu(setupCtx, h_params, d_params, true, air_instance_info->expressions_gpu, timer, stream);
    calculateWitnessSTD_gpu(setupCtx, h_params, d_params, false, air_instance_info->expressions_gpu, timer, stream);
