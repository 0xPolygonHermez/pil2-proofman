...
void genProof_gpu(SetupCtx& setupCtx, gl64_t *d_aux_trace, gl64_t *d_const_pols, gl64_t *d_const_tree, char *constTreePath, uint32_t stream_id, uint64_t instance_id, DeviceCommitBuffers *d_buffers, AirInstanceInfo *air_instance_info, bool skipRecalculation, TimerGPU &timer, cudaStream_t stream, bool recursive = false, bool reuse_constants = false) {
    ...
    uint64_t nFieldElements = setupCtx.starkInfo.starkStruct.verificationHashType == std::string("BN128") ? 1 : Poseidon2GoldilocksCommit::HASH_SIZE;
    ...
    if (recursive) {
        ...
        if (setupCtx.starkInfo.nPublics > 0)
        {
            ...
            else
            {
                ...
                d_transcript->put(h_params.challenges, Poseidon2GoldilocksCommit::HASH_SIZE, stream);
            }
        }
    } else {
    ...
    if(!setupCtx.starkInfo.starkStruct.hashCommits) {
        ...
        d_transcript->put(d_challenge, Poseidon2GoldilocksCommit::HASH_SIZE, stream);
    }
    ...
    for (uint64_t step = 0; step < setupCtx.starkInfo.starkStruct.steps.size(); step++)
    {
        ...
        {
            merkelizeFRI_inplace(setupCtx, h_params, step, d_friPol, starks.treesFRI[step], currentBits, setupCtx.starkInfo.starkStruct.steps[step + 1].nBits, setupCtx.starkInfo.starkStruct.merkleTreeArity, d_transcript, timer, stream);
        }
        else
        {
            if(!setupCtx.starkInfo.starkStruct.hashCommits) {
