...
void getHintFieldGPU(SetupCtx& setupCtx, StepsParams &h_params, StepsParams *d_params, HintFieldInfo *hintFieldValues, uint64_t hintId, std::string hintFieldName, HintFieldOptions& hintOptions, ExpressionsGPU* GPUExpressionsCtx, ExpsArguments *d_expsArgs, DestParamsGPU *d_destParams, Goldilocks::Element *pinned_exps_params, Goldilocks::Element *pinned_exps_args, uint64_t& countId, TimerGPU &timer, cudaStream_t stream);
