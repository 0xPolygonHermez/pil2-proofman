...
bool MerkleTreeGL::verifyGroupProof(Goldilocks::Element* root, Goldilocks::Element* level, std::vector<std::vector<Goldilocks::Element>> &mp, uint64_t idx, std::vector<Goldilocks::Element> &v) {
    ...
    switch(arity) {
        ...
        case 4:
            ...
            break;
        default:
            zklog.error("MerkleTreeGL::verifyGroupProof: Unsupported arity (STARK_POSEIDON1 supports 2, 3, 4)");
            exitProcess();
            exit(-1);
    }
#else
...
void MerkleTreeGL::calculateRootFromProof(Goldilocks::Element (&value)[4], std::vector<std::vector<Goldilocks::Element>> &mp, uint64_t &idx, uint64_t offset) {
    ...
    switch(arity) {
        ...
        default:
            zklog.error("MerkleTreeGL::calculateRootFromProof: Unsupported arity (STARK_POSEIDON1 supports 2, 3, 4)");
            exitProcess();
            exit(-1);
    }
#else
...
void MerkleTreeGL::merkelize()
{
    ...
    switch(arity) {
        ...
        case 4:
            ...
            break;
        default:
            zklog.error("MerkleTreeGL::merkelize: Unsupported arity (STARK_POSEIDON1 supports 2, 3, 4)");
            exitProcess();
            exit(-1);
    }
#else
