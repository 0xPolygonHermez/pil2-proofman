...
public:
    ...
    inline void loadPolynomials(StepsParams &params, ParserArgs &parserArgs, std::vector<Dest> &dests, Goldilocks::Element *bufferT_, uint64_t row, uint64_t domainSize, bool domainExtended)
    {
        ...
        {
            uint64_t opening = setupCtx.starkInfo.verify ? 0 : setupCtx.starkInfo.openingPoints[i] < 0 ? setupCtx.starkInfo.openingPoints[i] + domainSize
                                                                                                       : setupCtx.starkInfo.openingPoints[i];
    ...
    #pragma omp parallel for
        for (uint64_t i = 0; i < domainSize; i+= nrowsPack) {
            ...
            if (!compilation_time)
                loadPolynomials(params, parserArgs, dests, bufferT_, i, domainSize, domainExtended);

            Goldilocks::Element **destVals = new Goldilocks::Element *[dests.size()];

            for (uint64_t j = 0; j < dests.size(); ++j)
            {
                if (i >= dests[j].domainSize)
                    continue;
                        ...
                        switch (ops[kk]) {
                            ...
                            case 1: {
                                ...
                                if( i==debug_line && print){
                                    ...
                                    printf("Arguments: %lu %d %lu %d\n", expressions_params[args[i_args + 4]][(nColsStagesAcc[args[i_args + 5]] + args[i_args + 6]) * nrowsPack].fe, args[i_args + 7], expressions_params[args[i_args + 8]][(nColsStagesAcc[args[i_args + 9]] + args[i_args + 10]) * nrowsPack].fe, args[i_args + 11]);
