...
void mul_scatter_kernel_rows(const MulJobDev* __restrict__ jobs, uint32_t nJobs, MulBases bases,
    ...
    for (uint64_t it = 0; it < nIter; ++it) {
        ...
        for (uint32_t k = 0; k < nJobs; ++k) {
            ...
            uint64_t sel = 1;
            if (!j.selConstOne) {
                sel = mulEvalForm(j.sel, bases, row, rowMask);
                if (sel == 0) continue;
            }
...
void mul_scatter_kernel_tile(const MulJobDev* __restrict__ jobs, MulBases bases,
    ...
    uint64_t sel = 1;
    if (!j.selConstOne) {
        sel = mulEvalFormTile(j.sel, bases, localRow, globalRow, traceRows, fullRows);
        if (sel == 0) return;
    }
    if (j.hasBus && mulEvalFormTile(j.bus, bases, localRow, globalRow, traceRows, fullRows)
                        != (uint64_t)j.tableId) return;

    const uint64_t idx = mulAddFE(mulEvalFormTile(j.value, bases, localRow, globalRow, traceRows, fullRows),
                                  j.biasFE);
    if (idx >= j.nTableRows) {
        if (oob != nullptr) {
...
void mul_scatter_launch_rows(const MulJobDev* d_jobs, uint32_t nJobs, const uint64_t* const* bases_,
    ...
    mul_scatter_kernel_rows<<<blocks, MUL_SCATTER_BLOCK, 0, stream>>>(
        d_jobs, nJobs, bases, domainSize - 1, maxRows, acc, oob, air);
}
