...
//
// The accumulator is [column][row] because that is the order the scatter addresses it in: a table's
// consecutive rows are consecutive counters, so a warp's atomics land on the same cache lines. The
// committed trace is [row][column]. That transpose used to happen on the host, after a D2H of the
// whole accumulator and two more passes over it; here it is one kernel writing straight into the
// commit's destination, which is why the trace never crosses PCIe.
//
// Tiled through shared memory so both the read and the write are coalesced: a naive thread-per-
// element transpose makes one of the two strided. The +1 pad breaks shared-memory bank conflicts.
