...
inline void mul_register_vt(uint64_t airgroupId, uint64_t airId,
    ...
    // One of these per air with a virtual table -- not an operator's concern on a normal run (the
    // aggregate coverage line below is); kept at trace for whoever is debugging registration itself.
...
// Slots per map, stored. Deriving it from the buffer length is wrong the moment a layout has a
// header -- a packed map is `1 + nKey + slots*2` words, not `slots * (nKey + 1)` -- and the probe
// masks with `slots - 1`, so a derived value that is not a power of two silently misses every
// lookup. That mistake was made in three separate places; there is now one source for it.
...
inline void mul_register_table_map_impl(uint64_t tableId, const uint64_t* kv, uint64_t n,
    ...
    // The probe wraps with `slots - 1`, so anything but a power of two turns that mask into a few
    // stray bits and the lookup spins over the whole table. Caught here rather than as a hang.
    ...
    // The length is given, never re-derived: a packed map is `1 + nKey + slots*2` words, not
    // `slots * (nKey + 1)`, and computing it here read hundreds of megabytes past the buffer.
    ...
    // One of these per mapped table -- scales with the proving key, not with anything an operator
    // did, so it belongs at trace (the aggregate coverage line is the info-level summary).
...
inline void mul_register_table_digits_impl(uint64_t tableId, const uint64_t* tab, uint64_t n,
    ...
    // Per-table, like the map registration above -- trace, not info.
...
inline void mul_materialize_decoders() {
    for (const auto& L : mulVtLayouts()) {
        for (const auto& o : mulOwnedReqs()) {
            ...
            // One of these per decoder materialized -- scales with the proving key; the aggregate
            // coverage line (mul_log_coverage) is the info-level view of the same fact.
...
inline void mul_register_range_tables_impl(const uint64_t* tableIds, const int64_t* biases, uint64_t n) {
    ...
    // Superseded, for an operator, by the aggregate coverage line at the end of registration.
...
// What is still Rust-owned, with the height a decoder would have to cover. One aggregate line
// across every air with a virtual table, not one per air: an operator wants how much of the
