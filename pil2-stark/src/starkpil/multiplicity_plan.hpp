...
inline MulPlan mulBuildPlan(SetupCtx& setupCtx) {
    MulPlan plan;
    // Compiled programs, keyed by their instruction bytes. See `compile` below.
    ...
    for (uint64_t i = 0; i < n; ++i) {
        ...
        for (const auto& dec : mulDecoders()) {
            ...
            if (!feeds) continue;
            // A K-element tuple needs a row map of some kind: a fitted affine one, an exact map,
            // or a digit rule over its first column. A 1-element tuple is the range-check case and
            // uses `bias`. A tuple with none of these is nobody's to count here.
            ...
            // A mapped table does not use the folded `value` form at all -- it evaluates its key
            // columns -- so a value the extractor could not reduce is no reason to hand it to the
            // interpreter, which has no map support and would silently count nothing. Table 331
            // reached the fallback this way and contributed zero.
            ...
            if (dec.digitCols != 0) {
                // Only the columns the rule reads, in its own order -- that is the whole point of
                // pruning them: table 126's rule needs one of its six.
    ...
    // trace alone (nothing reads a later stage, which is true by construction at commit time).
    {
        ...
        // Group the jobs that share a compiled selector so the kernel's carry-across actually hits.
        // Only the order changes; every job still runs, and the accumulator takes atomic adds.
