        ...
        if(dests[0].params[0].parserParams.expId == int64_t(setupCtx.starkInfo.cExpId)) {
            ...
            for(uint64_t d = 0; d < setupCtx.starkInfo.openingPoints.size(); ++d) {
                for(uint64_t k = 0; k < FIELD_EXTENSION; ++k) {
                    Goldilocks::load_avx512(bufferT_[nColsStagesAcc[ns*nOpenings] + d*FIELD_EXTENSION + k], &xDivXSub[d*FIELD_EXTENSION + k], setupCtx.starkInfo.openingPoints.size()*FIELD_EXTENSION);
                }
            }   
