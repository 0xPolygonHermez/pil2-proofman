...
public:
    ...
    inline void load(__m512i *value, StepsParams& params, __m512i** expressions_params, uint16_t* args, uint64_t i_args, uint64_t row, uint64_t dim, uint64_t domainSize, bool domainExtended) {
        ...
        if (type == 0) {
            ...
            Goldilocks::load_avx512(x_avx, &proverHelpers.x[row]);
            Goldilocks3::op_31_avx512(3, (Goldilocks3::Element_avx512 &)value[0], (Goldilocks3::Element_avx512 &)value[0], x_avx);
            ...
            Goldilocks3::op_31_avx512(2, (Goldilocks3::Element_avx512 &)value[0], (Goldilocks3::Element_avx512 &)value[0], x_avx);
            ...
            if(dim == 1) {
                Goldilocks::copy_avx512(value[0], expressions_params[type][args[i_args + 1]]);
            } else {
                Goldilocks::copy_avx512(value[0], expressions_params[type][args[i_args + 1]]);
                Goldilocks::copy_avx512(value[1], expressions_params[type][args[i_args + 1] + 1]);
                Goldilocks::copy_avx512(value[2], expressions_params[type][args[i_args + 1] + 2]);
            }
        }
    }
    ...
    void calculateExpressions(StepsParams& params, ParserArgs &parserArgs, std::vector<Dest> dests, uint64_t domainSize, bool compilation_time) override {
        ...
        __m512i challenges[setupCtx.starkInfo.challengesMap.size()*FIELD_EXTENSION];
        ...
        __m512i evals[setupCtx.starkInfo.evMap.size()*FIELD_EXTENSION];
        ...
        for (uint64_t i = 0; i < domainSize; i+= nrowsPack) {
            ...
            for(uint64_t j = 0; j < dests.size(); ++j) {
                ...
                for(uint64_t k = 0; k < dests[j].params.size(); ++k) {
                    ...
                    expressions_params[buffer_types_size + 1] = (__m512i *)&tmp3_[omp_get_thread_num()*maxTemp3Size];

                    __m512i* a = &values_[omp_get_thread_num()*2*FIELD_EXTENSION];
                    __m512i* b = &values_[omp_get_thread_num()*2*FIELD_EXTENSION + FIELD_EXTENSION];
                    ...
                    for (uint64_t kk = 0; kk < dests[j].params[k].parserParams.nOps; ++kk) {
                        switch (ops[kk]) {
                            case 0: {
                                ...
                                Goldilocks::copy_avx512(expressions_params[buffer_types_size][args[i_args]], a[0]);
                                i_args += 4;
                                break;
                            }
                            case 1: {
                                ...
                                Goldilocks::op_avx512(args[i_args], expressions_params[buffer_types_size][args[i_args + 1]], a[0], b[0]);
                                i_args += 8;
                                break;
                            }
                            case 2: {
                                ...
                                load(b, params, expressions_params, args, i_args + 5, i, 1, domainSize, domainExtended);
                                Goldilocks3::op_31_avx512(args[i_args], (Goldilocks3::Element_avx512 &)expressions_params[buffer_types_size + 1][args[i_args + 1]], (Goldilocks3::Element_avx512 &)a[0], b[0]);
                                i_args += 8;
                                break;
                            }
                            case 3: {
                                ...
                                Goldilocks3::op_avx512(args[i_args], (Goldilocks3::Element_avx512 &)expressions_params[buffer_types_size + 1][args[i_args + 1]], (Goldilocks3::Element_avx512 &)a[0], (Goldilocks3::Element_avx512 &)b[0]);
                                i_args += 8;
                                break;
                            }
                            case 4: {
                                ...
                                Goldilocks3::copy_avx512((Goldilocks3::Element_avx512 &)expressions_params[buffer_types_size + 1][args[i_args]], (Goldilocks3::Element_avx512 &)b[0]);
                                i_args += 4;
                                break;
                            }
                            default: {
