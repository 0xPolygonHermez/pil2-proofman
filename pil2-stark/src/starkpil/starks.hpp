...
public:
    Starks(SetupCtx &setupCtx_, Goldilocks::Element *pConstPolsExtendedTreeAddress, bool initializeFRI = false) : setupCtx(setupCtx_)
    {
        ...
        for (uint64_t i = 0; i < setupCtx.starkInfo.nStages + 1; i++)
        {
            ...
            treesGL[i] = new MerkleTreeType(setupCtx.starkInfo.starkStruct.merkleTreeArity, setupCtx.starkInfo.starkStruct.merkleTreeCustom, 1 << setupCtx.starkInfo.starkStruct.nBitsExt, nCols, initializeFRI, allocateNodes || initializeFRI);
        }

        for (uint64_t i = 0; i < setupCtx.starkInfo.customCommits.size(); i++)
        {
            uint64_t nCols = setupCtx.starkInfo.mapSectionsN[setupCtx.starkInfo.customCommits[i].name + "0"];
            treesGL[setupCtx.starkInfo.nStages + 2 + i] = new MerkleTreeType(setupCtx.starkInfo.starkStruct.merkleTreeArity, setupCtx.starkInfo.starkStruct.merkleTreeCustom, 1 << setupCtx.starkInfo.starkStruct.nBitsExt, nCols, initializeFRI, allocateNodes || initializeFRI);
        }
        ...
        for (uint64_t step = 0; step < setupCtx.starkInfo.starkStruct.steps.size() - 1; ++step)
        {
            ...
            treesFRI[step] = new MerkleTreeType(setupCtx.starkInfo.starkStruct.merkleTreeArity, setupCtx.starkInfo.starkStruct.merkleTreeCustom, nGroups, groupSize * FIELD_EXTENSION, initializeFRI, allocateNodes || initializeFRI);
        }
    };
    ~Starks()
    {
