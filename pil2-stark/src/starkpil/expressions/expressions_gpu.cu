...
__device__ __noinline__ void scatterPolynomial__(ExpsArguments *d_expsArgs,
    ...
    uint64_t idx;
    // The outer gate must cover all three tuple-resolved shapes, not just mapSlots -- a digitCols-
    // or indexed-base-only decoder taking the `else` branch would run the single-value affine
    // decode against tuple element 0 instead of resolving the tuple, and count a wrong row.
    if (tupleForm) {
        // The interpreter evaluated the tuple; resolving it is the same step the kernel takes, so
        // the two paths cannot disagree about a row. Bound by nVal, not sc.dec.nKey -- a digitCols
        // decoder never sets nKey (it counts columns via digitCols instead), so nKey would read 0
        // and leave keyv uninitialised for that shape.
