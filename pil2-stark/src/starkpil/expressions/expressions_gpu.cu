...
// Element* arithmetic would stride 8x too far. Bounded by the smaller allocator.
...
{
    if (countId >= PINNED_EXPS_SLOTS_RECURSIVEF) {
        zklog.error("ExpressionsGPU: expression launch count " + std::to_string(countId) +
                    " exceeds pinned slot capacity " + std::to_string(PINNED_EXPS_SLOTS_RECURSIVEF));
        exitProcess();
    }
