...
// Block-level combining for the scatter's atomics.
//
// Why: the counts are not spread over their tables, they pile onto a few counters. Measured on
// zec-reth, table 102 has 16.7M counters but only 69,539 are ever touched and ONE of them takes
// 131M of its 943M increments; table 104 takes 519M increments over 65K counters with one at 260M.
// Warp aggregation already collapses lanes within a warp, but thousands of warps still serialise on
// those same addresses -- removing the atomics entirely took the scatter from 138 ms to 34 ms, so
// three quarters of the cost is that serialisation.
//
