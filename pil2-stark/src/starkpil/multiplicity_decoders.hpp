...
// uses an exact map derived from the table's COL_* columns.
...
struct MulDecoder {
    ...
    // Exact-match map: open addressing over slots of [key columns..., row],
...
// Key -> row. Without a map the key IS the row. Returns false when the tuple is not in the table;
// the caller records an out-of-range decode.
