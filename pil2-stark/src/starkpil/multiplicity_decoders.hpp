...
MUL_HD inline bool mulResolveRow(const uint64_t* key, uint32_t nKey, uint64_t mapSlots,
    ...
    if (digitCols != 0) {
        // The key holds exactly the columns the rule reads, in its own order; the plan evaluates
        // only those. A digit the table never holds leaves the sum invalid, so the caller records
        // an out-of-range decode instead of counting a wrong row.
        uint64_t sum = 0;
        for (uint32_t c = 0; c < digitCols; ++c) {
            const uint64_t base = digitTab[c];
            const uint64_t nd   = digitTab[digitCols + c];
            // A malformed digit table (base < 2, or no digits at all) would otherwise divide by
            // zero here instead of missing cleanly like every other decode path in this file.
    ...
    // Packed keys: the tuple fits in one word, so a slot is two words instead of nKey+1 and the
    // probe compares one. Table 125's map is 1073 MB verbatim and 268 MB packed, and four slots
    // share a cache line instead of one slot spanning it. The shape lives in the map's header, so
    // the job carries nothing extra.
    if (mapKV[0] == MUL_MAP_PACKED) {
        ...
        for (uint32_t c = 0; c < nKey; ++c) {
            ...
            // Wider than this column ever holds, so the tuple is not in the table. Without this a
            // stray high bit would land on another entry's key.
        ...
        // Bounded. An unbounded walk over a 16.7M-slot table is a kernel that holds its stream for
        // minutes -- indistinguishable from a hang, and it starves every other proof behind it.
        // Measured chains are 50 for a hit and 77 for a miss, so this only ever trips on a fault,
        // and the caller records it as an out-of-range decode rather than counting a wrong row.
    ...
    uint64_t i = h & (mapSlots - 1);
    // Bounded for the same reason the packed probe is: an unbounded walk over a multi-million-slot
    // map is a kernel that holds its stream for minutes rather than a wrong answer.
    const uint64_t limit = mapSlots < MUL_MAP_MAX_PROBE ? mapSlots : MUL_MAP_MAX_PROBE;
    for (uint64_t probe = 0; probe < limit; ++probe) {
...
// {count, table_id, air, row} for the first decode that landed outside its table, plus (for a
// multi-column miss) the full key that produced it: slots 4..14 hold up to 11 key elements and
// slot 15 holds how many of those are meaningful.
...
// the first, so a wrong decoder is diagnosable without syncing the stream. `key` is the full tuple
// that missed (or a single-element array holding just the resolved row, for the affine case) --
// logging only key[0] loses exactly the columns a multi-column decoder bug needs to be diagnosed.
