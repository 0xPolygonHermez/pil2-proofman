...
namespace PilFflonkZkey
{
    PilFflonkZkey::~PilFflonkZkey()
    {
        ...
        // Each entry was allocated by loadPilFflonkZkey, along with the arrays
        // it points at; clear() alone drops the pointers and leaks the lot.
        for (auto &entry : f) {
            if (entry.second == nullptr) continue;
            for (u_int32_t i = 0; i < entry.second->nStages; i++) {
                delete[] entry.second->stages[i].pols;
            }
            delete[] entry.second->stages;
            delete[] entry.second->pols;
            delete[] entry.second->openingPoints;
            delete entry.second;
        }
        f.clear();
        ...
        // X2 is not ours to free: loadPilFflonkZkey sets it from
        // BinFile::read, which returns a pointer into the mapped file rather
        // than an allocation. Deleting it aborts with "munmap_chunk(): invalid
        // pointer" the first time a key is destroyed.
    }
